...
m_bass_note_number(0), m_amplitude_decay_constant(0)
{
  TRACE;
#define TODO123 0
  ...
  if(NULL != mB31EState) {
    ...
    ("Amplitude decay constant", m_amplitude_decay_constant,
     B31E_AMPLITUDE_DECAY_MIN, B31E_AMPLITUDE_DECAY_MAX, 0.01, "", "Bass");
...
void sines::advance() {
  while (!mMidiQueue.Empty()) {
    ...
    if (status == IMidiMsg::kNoteOff && m_bass_note_number == noteNumber) {
      ...
      sines_trigger_bass(mB31EState, freq, (velocity / 127.0), m_amplitude_decay_constant);
    }
    mMidiQueue.Remove();
  }
  mOffset++;
